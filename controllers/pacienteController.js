...
exports.createPaciente = async (req, res) => {
  const { nombre, edad, email, direccion, telefono } = req.body;

  try {
    // Verificar si los campos requeridos están presentes
    if (!nombre || !edad || !email || !direccion || !telefono) {
      return res.status(400).json({ error: 'Faltan campos obligatorios' });
    }

    // Crear el paciente en la base de datos
    const paciente = await Paciente.create({ nombre, edad, email, direccion, telefono });
